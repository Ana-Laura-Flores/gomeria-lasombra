...
import {
  getOrdenesTrabajo,
  getPagosPorMes, // o idealmente getPagosByCliente(clienteId)
} from "../services/api";
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const ordenesCCCliente = resOrdenes.data
        ...
        .filter((o) => o.cliente && o.cliente.id === clienteId);
      ...
      setClienteNombre(ordenesCCCliente[0]?.cliente?.nombre || pagosConfirmadosCliente[0]?.cliente?.nombre || "");
    } catch (err) {
  ...
  return (
    <MainLayout>
      ...
      <div className="hidden">
        <div id="cc-pdf">
          <CuentaCorrientePDF cliente={{ id: clienteId, nombre: clienteNombre }} movimientos={movimientos} />
        </div>
      </div>
    </MainLayout>
  );
